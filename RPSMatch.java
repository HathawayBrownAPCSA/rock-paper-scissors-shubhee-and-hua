...
/**
 * Runs a rock paper scissors match without cheats
 *
 * @Shubhee @Hua
 * @version 20211006
 */
public class RPSMatch
{
    ...
    /**Constructor for RPSMatch
     * @param humanScore, computerScore, ties
    */
    public RPSMatch(int hSc, int cSc, int t)
    {
       humanWins = hSc;
       computerWins = cSc;
       ties = t;
    ...
    {
        return getComputerPlay(0);
    }
    
    /**gets the computer's play
     * @param num determines which version of match
     * @return computerPlay
     */
    public String getComputerPlay (int num)
    {
        if(num == 0)
        {//no cheats
        ...
        if(num == 1)//actual cheat-cycle
        {
            if(prevHumanPlay.length() > 0 && (int)(Math.random() * 3) != 1)
            {
                computerPlay = prevHumanPlay;
                return computerPlay;
            }
            else
            {
